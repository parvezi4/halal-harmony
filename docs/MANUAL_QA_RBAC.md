...
| `ops.admin@example.com` | `Password123!` | ADMIN | `/admin/login` |
| `moderator@example.com` | `Password123!` | MODERATOR | `/admin/login` |
...
5. **Expected**: `ops.admin@example.com` (ADMIN) shows a Delete button
6. **Expected**: `moderator@example.com` (MODERATOR) shows a Delete button
...
1. Login as `ops.admin@example.com` (ADMIN)
2. Go to `/admin/moderation/settings` → Privileged Users
3. **Expected**:
   - `admin@example.com` (SUPERADMIN): shows "Protected" — no Delete button
   - `ops.admin@example.com` (self): **no Delete button** (cannot delete self)
   - `moderator@example.com` (MODERATOR): shows a Delete button
...
4. Logout; login as `moderator@example.com`
5. **Expected**:
...
2. Login as `moderator@example.com`
...
1. Login as `moderator@example.com` (with all capabilities enabled)
